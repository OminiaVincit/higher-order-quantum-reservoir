...
def nmse_job(qparams, nqrc, buffer, train_input_seq, train_output_seq, \
    ...
    for n in range(Ntrials):
         _, train_loss, _, val_loss = qm.get_loss(qparams, buffer, train_input_seq, train_output_seq, val_input_seq, val_output_seq, nqrc)
    ...
    rstr = '{} {} {} {} {} {} {}'.format(\
        qparams.tau_delta, qparams.virtual_nodes, qparams.max_coupling_energy, \
...
if __name__  == '__main__':
    ...
    parser.add_argument('--savedir', type=str, default='resnarma_multiplex')
    ...
    nproc, nqrc, tau_delta = args.nproc, args.nqrc, args.taudelta
    init_rho = args.rho
    ...
    outbase = os.path.join(savedir, '{}_{}_nq_{}_tdt_{}_order_{}_ntrials_{}'.format(basename, datestr, \
        nqrc, tau_delta, '_'.join([str(o) for o in orders]), Ntrials ))
    ...
    if os.path.isfile(savedir) == False:
        ...
        for V in virtuals:
            ...
            p = multiprocessing.Process(target=nmse_job, args=(qparams, nqrc, buffer, train_input_seq, train_output_seq, \
                val_input_seq, val_output_seq, Ntrials, send_end))
            jobs.append(p)
