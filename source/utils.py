...
def plot_predict_multi(input_sequence, output_sequence_multi, prediction_sequence_multi):
    cmap = plt.get_cmap("tab10")
    N = len(output_sequence_multi)
    plt.figure(figsize=(12,2*N))
    
    for index in range(N):
        plt.subplot(N,1,index+1)
        #plt.plot(input_sequence_list[index][1000:1100],color=cmap(index),linestyle="--",label="input")
        plt.plot(output_sequence_multi[index][1000:1200],color=cmap(index),linestyle=":",label="label")
        plt.plot(prediction_sequence_multi[index][1000:1200],color=cmap(index),label="prediction")
        plt.legend()
    plt.show()