...
def fft_param(model, num_units, cplex, dtype, dtypec):
    ...
    theta = tf.compat.v1.get_variable("theta_layer_{:d}".format(model._ln), [capacity, num_units//2], 
        initializer=phase_init, dtype=dtype)
    cos_theta = tf.cos(theta)
    sin_theta = tf.sin(theta)
        
    if cplex:
        phi = tf.compat.v1.get_variable("phi_layer_{:d}".format(model._ln), [capacity, num_units//2], 
            initializer=phase_init, dtype=dtype)
        cos_phi = tf.cos(phi)
        sin_phi = tf.sin(phi)
    ...
    if cplex:
        omega = tf.compat.v1.get_variable("omega_layer_{:d}".format(model._ln), [num_units], 
                initializer=phase_init, dtype=dtype)
...
def tunable_param(model, num_units, cplex, capacity, dtype, dtypec):
    ...
    phase_init = tf.compat.v1.random_uniform_initializer(-3.14, 3.14, dtype=dtype)

    theta_A = tf.compat.v1.get_variable("theta_A_layer_{:d}".format(model._ln), [capacity_A, num_units//2], 
        initializer=phase_init, dtype=dtype)
    cos_theta_A = tf.cos(theta_A)
    sin_theta_A = tf.sin(theta_A)

    if cplex:
        phi_A = tf.compat.v1.get_variable("phi_A_layer_{:d}".format(model._ln), [capacity_A, num_units//2], 
            initializer=phase_init, dtype=dtype)
    ...
    theta_B = tf.compat.v1.get_variable("theta_B_layer_{:d}".format(model._ln), [capacity_B, num_units//2 - 1], 
        initializer=phase_init, dtype=dtype)
    cos_theta_B = tf.cos(theta_B)
    sin_theta_B = tf.sin(theta_B)

    if cplex:
        phi_B = tf.compat.v1.get_variable("phi_B_layer_{:d}".format(model._ln), [capacity_B, num_units//2 - 1], 
            initializer=phase_init, dtype=dtype)
...
class EUNNCell(rnn_cell_impl.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None, reuse=None):
        with tf.compat.v1.variable_scope(scope or "eunn_cell", reuse=reuse):
            ...
            if self._cplex:
                U_re = tf.compat.v1.get_variable("U_re_layer_{:d}".format(self._ln), [inputs_size, self._num_units], initializer=input_matrix_init, dtype=self._dtype)
                U_im = tf.compat.v1.get_variable("U_im_layer_{:d}".format(self._ln), [inputs_size, self._num_units], initializer=input_matrix_init, dtype=self._dtype)
                inputs_re = tf.matmul(inputs, U_re)
                inputs_im = tf.matmul(inputs, U_im)
                inputs = tf.complex(inputs_re, inputs_im)
            else:
                U = tf.compat.v1.get_variable("U_layer_{:d}".format(self._ln), [inputs_size, self._num_units], 
            ...
            # activation
            bias = tf.compat.v1.get_variable("mod_ReLU_bias_layer_{:d}".format(self._ln), [self._num_units], 
                    initializer=tf.compat.v1.constant_initializer(), dtype=self._dtype)
