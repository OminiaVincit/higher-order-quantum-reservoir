...
tf_initializers = {"xavier": tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), "normal": tf.compat.v1.truncated_normal_initializer()}
...
class rnn_statefull_parallel(object):
    ...
    def rnn_layer(self, cell_type, name, input, input_size, num_hidden, activation_str, ln, initial_hidden_state):
        ...
        with tf.compat.v1.variable_scope(scope_name, initializer = tf_initializers[self.initializer]):
            # 'input' is a tensor of shape [batch_size, max_time, input_dim]
            if cell_type == 'lstm':
                cell = tf.compat.v1.nn.rnn_cell.LSTMCell(name='basic_lstm_cell', num_units=num_hidden, forget_bias=1.0, state_is_tuple = True, activation=tf_activations[activation_str])
                cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self._dropout_keep_prob)
                cell = ZoneoutWrapper(cell, zoneout_drop_prob=1-self._zoneout_keep_prob, is_training=self._is_training, dtype=self.dtype_tf)
            elif cell_type == 'gru':
                cell = tf.compat.v1.nn.rnn_cell.GRUCell(name='gru_cell', num_units=num_hidden, activation=tf_activations[activation_str])
                cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self._dropout_keep_prob)
                cell = ZoneoutWrapper(cell, zoneout_drop_prob=1-self._zoneout_keep_prob, is_training=self._is_training, dtype=self.dtype_tf)
            elif cell_type == 'plain':
                cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(num_units=num_hidden, activation=tf_activations[activation_str])
                cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self._dropout_keep_prob)
                cell = ZoneoutWrapper(cell, zoneout_drop_prob=1-self._zoneout_keep_prob, is_training=self._is_training, dtype=self.dtype_tf)
            ...
            outputs, last_state = tf.compat.v1.nn.dynamic_rnn(cell, input, dtype=self.cdtype_tf, initial_state=initial_hidden_state)
    ...
    def mlp_unfolded_layer(self, name, input, input_size, layer_size, activation_str, ln):
        ...
        # w shape [H, O]
        w = tf.compat.v1.get_variable(name + "_w_"+str(ln), [input_size, layer_size], dtype=self.dtype_tf, initializer=tf_initializers[self.initializer])
        # b shape [1, O]
        b = tf.compat.v1.get_variable(name + "_b_"+str(ln), [1, layer_size], dtype=self.dtype_tf, initializer=tf_initializers[self.initializer])
    ...
    def createModelName(self, params):
        ...
        str_ = "GPU-" * self.GPU + self.rnn_cell_type + "-PL"
        for key in keys:
    ...
    def __init__(self, params):
        ...
        self.input = tf.compat.v1.placeholder(self.dtype_tf, shape=[None, None, self.input_dim])
        self.target = tf.compat.v1.placeholder(self.dtype_tf, shape=[None, None, self.output_dim])
        self._dropout_keep_prob = tf.compat.v1.placeholder(self.dtype_tf, shape=())
        self._zoneout_keep_prob = tf.compat.v1.placeholder(self.dtype_tf, shape=())
        self._is_training = tf.compat.v1.placeholder(tf.bool, shape=())
        
        self.initial_hidden_states = []
        for ln in range(self.rnn_num_layers):
            if self.rnn_cell_type == 'lstm':
                c_state = tf.compat.v1.placeholder(self.cdtype_tf, shape=[None, self.rnn_size_layers])
                h_state = tf.compat.v1.placeholder(self.cdtype_tf, shape=[None, self.rnn_size_layers])
                initial_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c_state, h_state)
                self.initial_hidden_states.append(initial_state)
            else:
                self.initial_hidden_states.append(tf.compat.v1.placeholder(self.cdtype_tf, shape=[None, self.rnn_size_layers]))
        ...
        # MSE Loss of the rnn
        self.rmse_loss = tf.reduce_mean(input_tensor=tf.math.squared_difference(self.rnn_output_loss, self.target))
    ...
    def defineTrainer(self, vars):
        with tf.compat.v1.name_scope('Optimizer_Scope'):
            # optimizer = tf.train.GradientDescentOptimizer(learning_rate=self.learning_rate)
            # optimizer = tf.contrib.opt.NadamOptimizer(learning_rate=self.learning_rate, beta1=0.9, beta2=0.999)
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate_round, beta1=0.9, beta2=0.999)
            # Op to calculate every variable gradient
            grads = tf.gradients(ys=self.loss, xs=vars)
    ...
    def trainRound(self, round_num, idx_train_on, idx_val_on, n_train_samples, n_val_samples, train_input_sequence, val_input_sequence):
        ...
        saver = tf.compat.v1.train.Saver(max_to_keep=100000, var_list=self.trainable_variables)
        ...
        optimizer_scope = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                 "Optimizer_Scope")
        reset_opt_op = tf.compat.v1.variables_initializer(optimizer_scope)
        ...
        if self.retrain == 1 or round_num>0:
            ...
            self.trainer_reset = tf.compat.v1.variables_initializer(self.optimizer.variables())
