...
class ZoneoutWrapper(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def zoneoutLoop(self, state, new_state):
        ...
        for i, state_element in enumerate(state):
            ...
            random_tensor += tf.random.uniform(tf.shape(input=state_element), dtype=self._dtype)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        final_new_state = tf.cond(pred=self._is_training, true_fn=lambda: self.zoneoutLoop(state, new_state), false_fn=lambda: self.normalLoop(state, new_state))

        if isinstance(self._cell.state_size, tuple):
            return output, tf.compat.v1.nn.rnn_cell.LSTMStateTuple(final_new_state[0], final_new_state[1])
