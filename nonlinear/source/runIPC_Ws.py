...
def IPC_compute(qparams, ipcparams, length, ntrials, ranseed, log_filename, savedir, posfix, \
    log_Ws, gamma, nqrc, combine_input, type_input, type_op, input_file):
    logger = get_module_logger(__name__, log_filename)
    logger.info(log_filename)
    
    for log_W in log_Ws:
        non_diag_var = 10**log_W
        qparams.non_diag_var = non_diag_var
        hqrc.get_IPC(qparams, ipcparams, length, nqrc=nqrc, gamma=gamma, logger=logger, ranseed=ranseed, Ntrials=ntrials, \
            savedir=savedir, posfix='capa_logW_{:.3f}_{}'.format(log_W, posfix), feed_nothing=False, nonlinear=0, dim_input=1,\
            type_op=type_op, type_input=type_input, combine_input=combine_input, label='logW_{:.3f}'.format(log_W), input_file=input_file)
...
if __name__  == '__main__':
    ...
    type_input, type_op, non_diag_const, gamma = args.type_input, args.type_op, args.non_diag_const, args.gamma
    ...
    basename = '{}_{}_{}_nqrc_{}_nspins_{}_seed_{}_mdeg_{}_mvar_{}_thres_{}_delays_{}_T_{}_cb_{}_gam_{}_op_{}_tp_{}'.format(\
        basename, dynamic, solver, nqrc, n_spins, ranseed, max_deg, max_num_var, thres, \
        args.deg_delays, length, combine_input, gamma, type_op, type_input)
        ...
        for V in virtuals:
            for tau in taus:
                ...
                for pid in range(nproc):
                    ...
                    sub_log_filename = os.path.join(logdir, 'logW_{:.3f}_{:.3f}_{}.log'.format(log_Ws[0], log_Ws[-1], posfix))
                    ...
                    p = multiprocessing.Process(target=IPC_compute, \
                        args=(qparam_p, ipcparams, length, ntrials, ranseed, sub_log_filename, \
                            savedir, posfix, log_Ws, gamma, nqrc, combine_input, type_input, type_op, input_file))
                    jobs.append(p)
