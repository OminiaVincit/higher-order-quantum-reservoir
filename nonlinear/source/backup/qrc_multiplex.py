...
class SpatialQuantumReservoirComputing(object):
    def __init_reservoir(self, qparams, nqrc):
        I = [[1,0],[0,1]]
        Z = [[1,0],[0,-1]]
        X = [[0,1],[1,0]]
        P0 = [[1,0],[0,0]]
    ...
    def train_to_predict(self, input_sequence, output_sequence, buffer, qparams, nqrc):
        self.__init_reservoir(qparams, nqrc)
    ...
    def init_forward(self, qparams, input_seq, nqrc, init_rs):
        if init_rs == True:
            self.__init_reservoir(qparams, nqrc)
        _, state_list =  self.__feed_forward(input_seq, predict=False)
        return state_list

def get_loss(qrcparams, buffer, train_input_seq, train_output_seq, val_input_seq, val_output_seq, nqrc):
    model = SpatialQuantumReservoirComputing()

    train_input_seq = np.array(train_input_seq)
    train_output_seq = np.array(train_output_seq)
    model.train_to_predict(train_input_seq, train_output_seq, buffer, qrcparams, nqrc)
