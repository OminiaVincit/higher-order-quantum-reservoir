...
def dump_reservoir_states(logger, datfile, Xs, qparams, model, init_rs, ranseed):
    if os.path.isfile(datfile) == True:
        ...
        # Pickle: cannot dump file larger 4GB
        #with open(datfile, 'wb') as wrs:
        #    pickle.dump(rstates, wrs)
        np.save(datfile, rstates)
        logger.debug('Dumped npy file {}'.format(datfile))
    return datfile
...
if __name__  == '__main__':
    ...
    for n in range(ntrials):
        ...
        # permute data
        train_idx = np.random.permutation(len(y_train_lb))
        test_idx = np.random.permutation(len(y_test_lb))
        ...
        # Combine data into a large time series
        ...
        for datlb in ['train', 'test']:
            ...
            for alpha in strengths:
                for tau in taudeltas:
                    ...
                    model.init_reservoir(qparams, ranseed=ranseed)
                    init_rs, rs_seed, use_lastrho = False, 0, False
                    # create training and testing data
                    datfile = '{}_{}_alpha_{:.2f}_tau_{:.4f}_shape_{}_{}.npy'.format(datlb, basename, alpha, tau, ns, ds)
        ...
        for alpha in strengths:
            for tau in taudeltas:
                if linear_reg > 0:
                    ...
                    # Training
                    trainfile = datfs['train_alpha_{:.2f}_tau_{:.4f}'.format(alpha, tau)]
                ...
                if linear_reg > 0:
                    ...
                    testfile = datfs['test_alpha_{:.2f}_tau_{:.4f}'.format(alpha, tau)]
