...
def IPC_compute(qparams, ipcparams, length, ntrials, ranseed, log_filename, savedir, posfix, nqrc, alphas, explb, mask_input, combine_input, qr_input):
    ...
    for alpha in alphas:
        ...
        hqrc.get_IPC(qparams, ipcparams, length, nqrc=nqrc, gamma=gamma, logger=logger, ranseed=ranseed, Ntrials=ntrials, \
            savedir=savedir, posfix='capa_alpha_{:.3f}_{}'.format(alpha, posfix), feed_nothing=False, nonlinear=0, dim_input=qr_input,\
...
if __name__  == '__main__':
    ...
    basename = '{}_{}_{}_nqrc_{}_nspins_{}_amax_{}_amin_{}_nas_{}_seed_{}_mdeg_{}_mvar_{}_thres_{}_delays_{}_T_{}_mask_{}_cb_{}_qrin_{}'.format(\
        basename, dynamic, solver, nqrc, n_spins, amax, amin, nas, ranseed, max_deg, max_num_var, thres, args.deg_delays, length, mask_input, combine_input, qr_input)
            ...
            for tau in taus:
                ...
                for pid in range(nproc):
                    ...
                    p = multiprocessing.Process(target=IPC_compute, \
                        args=(qparams, ipcparams, length, ntrials, ranseed, log_filename, savedir, posfix, nqrc, tBs, explb, mask_input, combine_input, qr_input))
                    jobs.append(p)
