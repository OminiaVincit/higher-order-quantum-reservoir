...
if __name__  == '__main__':
    ...
    print(args)
    folder, prefix, posfix, tau, nqrc, Ntrials = args.folder, args.prefix, args.posfix, args.tau, args.nqrc, args.ntrials
    orders = [5,10,15,20]
    N = len(orders)
    ...
    for i in range(N):
        ...
        for deep in [0]:
            ncl = 0
            for V in [1,5,10,15]:
                for rfile in glob.glob('{}/{}*V_{}_QRs_{}_*narma_{}_deep_{}_ntrials_{}_*_{}.txt'.format(folder, prefix, \
                    V, nqrc, order, deep, Ntrials, posfix)):
                    print(rfile)
                    ...
                    for ninput in range(1,nqrc+1):
                        ...
                        if False:
                            ...
                            ax.plot(xa[sids], ya[sids], lstype[deep], color=col, alpha = 0.8, linewidth=2.0, mec='k', mew=0.5, markersize=8, label='V={}; M={}'.format(V,ninput))
                            #ax.fill_between(xa[sids], ya[sids] - za[sids], ya[sids] + za[sids], facecolor=col, alpha=0.2)
                        
            
            ax.set_xlabel('Scaling input', fontsize=20)
            ...
            #ax.set_ylim([np.min(avg_tests[id1])/1.2, np.max(avg_tests[id1])*1.01])
