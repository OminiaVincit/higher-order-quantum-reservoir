...
for V in 10 20
do
python $BIN --input_type $INPUT_TYPE --taudeltas $TAUS --width $WIDTH --dynamic $DYNAMIC --non_diags $DS --ntrials $N --transient $TRANS --rate $RATE --linear_reg $LN --mnist_size $MNIST_SIZE --savedir $SAVE --nqrs $NQR --virtuals $V --full $FULL --label1 $LB1 --label2 $LB2 --strengths $STRENGTHS --use_corr $CORR
done
done
done